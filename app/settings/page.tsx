    ...
    <div className="grid gap-6">
      <section className="rounded-[2rem] border border-stone-200 bg-white p-8 shadow-[0_24px_60px_-32px_rgba(41,37,36,0.35)]">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-7 text-stone-600">
          Cloudflare R2 credentials live in `.env.local` on the server. ChatPDF API keys are intentionally not stored on the server:
        ...
        <div className="mt-6 grid gap-4 rounded-[1.5rem] border border-stone-200 bg-stone-50 p-5 text-sm leading-7 text-stone-700">
          <p>Create a `.env.local` file from `.env.example` and add your R2 values.</p>
...
R2_CATALOG_KEY=catalog/catalog.json`}
          ...
          <p>
            After that, restart the dev server and use the Upload page. When you open a chapter, the reader streams the PDF through the app and the
